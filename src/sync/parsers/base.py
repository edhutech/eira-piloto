...
from ..models import DiscardedDocumentEvent, DriveFile, NormalizedEvent
...
def event_id(file_id: str, source_locator: str, participant: str, raw_text: str) -> str:
    value = "\x1f".join((file_id, source_locator, participant, raw_text)).encode("utf-8")
    return hashlib.sha256(value).hexdigest()


def speaker_events(text: str, file_id: str, session_number: int, channel: Literal["voice", "chat"]) -> list[NormalizedEvent]:
    lines = text.replace("\r\n", "\n").replace("\r", "\n").split("\n")
    ...
    def emit(value: dict[str, Any]) -> None:
        ...
        value["event_id"] = event_id(value["source_file_id"], value["source_locator"], value["participant_raw"], raw)
