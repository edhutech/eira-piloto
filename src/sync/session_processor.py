...
from .countability import COUNTABILITY_RULESET_VERSION, classify_event
from .models import DriveFile, SessionInspection
from .participants import Participant, ParticipantResolver, ResolutionStatus
from .scoring import ParticipantSessionScore, score_events
from .session_results_repository import ParticipantSnapshot
...
class ContentLoader(Protocol):
    def read(self, file: DriveFile) -> Any: ...
...
class SessionProcessor:
    ...
    content_loader: ContentLoader | Callable[[DriveFile], Any]
    ...
    def process(self, inspection: SessionInspection) -> SessionProcessResult:
        ...
        try:
            ...
            transcript_files = tuple(item[0].file_id for item in parsed if item[1].artifact_type == "transcript")
            chat_files = tuple(item[0].file_id for item in parsed if item[1].artifact_type == "chat")
    ...
    def _parse(self, inspection: SessionInspection, warnings: list[str]) -> list[tuple[DriveFile, Any]]:
        parsed: list[tuple[DriveFile, Any]] = []
        for file in sorted(inspection.files, key=lambda item: (item.file_id, item.name)):
            ...
            if parser is None:
                warnings.append(f"Archivo no soportado omitido: {file.file_id}")
                continue
    ...
    def _event_sort_key(event: Any) -> tuple[str, str, str]:
        return (str(getattr(event, "source_file_id", "")), str(getattr(event, "source_locator", "")), str(getattr(event, "event_id", "")))
