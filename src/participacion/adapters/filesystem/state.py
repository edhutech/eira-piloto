...
DEFAULT_STATE_PATH = Path(os.environ.get(
    "PARTICIPACION_STATE_DIR", Path.home() / ".local" / "state" / "participacion"
)) / "sync_state.json"
...
def stored_file_fingerprint(state: dict[str, Any], program_id: str, session_id: str, artifact_id: str) -> str | None:
    return state.get("programs", {}).get(program_id, {}).get("sessions", {}).get(session_id, {}).get("files", {}).get(artifact_id)
